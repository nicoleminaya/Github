...
import os
import glob
import optuna # Used to TPE method
import numpy as np
import pandas as pd
import torch
...
pathToDB    = os.path.join(pathToRoot, 'data', 'db_' + wds_name)#+'_'+ args.db) # Generated data
pathToWDS   = os.path.join('water_networks', wds_name+'.inp')                  # WDN topology
pathToLog   = os.path.join('experiments', 'hyperparams',wds_name, '_ho.pkl')
...
def objective(trial):
    ...
    for i in range(n_layers):
        topo.append([
            ...
            trial.suggest_int('filter_size_{}'.format(i), 5, 50, step=5) # K was set to 30, but in the paper is said to 50
            ])
...
if __name__ == '__main__':
    ...
    study   = optuna.create_study(direction='minimize',
        study_name  = 'v4',
        sampler = sampler,
        storage = 'sqlite:///experiments/hyperparams/'+str(wds_name)+'_ho-'+str(args.obsrat)+'.db'
        )
